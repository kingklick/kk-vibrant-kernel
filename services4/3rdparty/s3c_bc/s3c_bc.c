                                    ...
                                    IMG_BOOL        *pbIsContiguous)
{
	...
	*pui32ByteSize = S3C_BC_DEVICE_BUF_SIZE;
	*ppvCpuVAddr = 0;
	*phOSMapInfo = 0;
...
S3C_BC_ERROR S3C_BC_Register(void)
{
	...
	if (!psDevInfo)
	{
		...
		if(!psDevInfo)
		{
			return (S3C_BC_ERROR_OUT_OF_MEMORY);
		}
		...
		psDevInfo->sBufferInfo.pixelformat        = S3C_BC_DEVICE_PIXELFORMAT;
		psDevInfo->sBufferInfo.ui32Width          = S3C_BC_DEVICE_WIDTH;
		psDevInfo->sBufferInfo.ui32Height         = S3C_BC_DEVICE_HEIGHT;
		psDevInfo->sBufferInfo.ui32ByteStride     = S3C_BC_DEVICE_STRIDE;
		...
		for(i = 0; i < S3C_BC_DEVICE_BUFFER_COUNT; i++, addr += S3C_BC_DEVICE_BUF_SIZE)
		{
